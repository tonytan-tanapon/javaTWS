...
class OptionChainsPanel extends JPanel {
	...
	OptionChainsPanel() {
		...
		m_lastTradeDateOrContractMonth.setText("20230127");
		...
		API.INSTANCE.m_controller.reqMktDataType( MarketDataType.getField(m_marketDataType.getSelectedItem())));
	...
	void onAdd() {
		...
		API.INSTANCE.m_controller.reqContractDetails( m_underContract, this::onRecUnderDetails);
	}

	void onRecUnderDetails(List<ContractDetails> list) {
		if (list.size() != 1) {
			API.INSTANCE.show( "Error: " + list.size() + " underlying contracts returned");
			return;
		}
		...
		API.INSTANCE.m_controller.reqContractDetails( optContract, symbolPanel);
	}
	...
	private class ChainPanel extends NewTabPanel implements IContractDetailsHandler, ActionListener  {
		...
		ChainPanel() {
			...
			API.INSTANCE.m_controller.reqTopMktData( m_underContract, "", false, false, m_stockListener);
		}
        ...
        @Override public void closed() {
        	API.INSTANCE.m_controller.cancelTopMktData( m_stockListener);
		...
		List<ContractDetails> list_option = new ArrayList<ContractDetails>();
...
//			System.out.println(list.toString());
			...
			for (ContractDetails data : list) {
				...
				//System.out.println(data.contract().symbol()+" "+data.contract().getRight()+" "+data.contract().lastTradeDateOrContractMonth()+" "+data.contract().strike());
		...
		private class ChainModel extends AbstractTableModel {
            ...
            void desubscribe() {
                for (ChainRow row : m_list) {
                	API.INSTANCE.m_controller.cancelOptionMktData( row);
                }
            }
			...
			void addRow(Contract contract, boolean snapshot) {
				...
				API.INSTANCE.m_controller.reqOptionMktData(contract, "", snapshot, false, row);
