...
class OptionChainsPanelTrade2 extends JPanel {
	...
	JLabel showContract = new JLabel("Contract");
	JLabel showStrike = new JLabel("Strike");
	JLabel showPrice = new JLabel("Price");
	OptionChainsPanelTrade2() {
...
//		add( m_tabbedPanel);
		...
		JTextField show = new JTextField("Hello world");
		
		pNorth.setLayout( new BorderLayout());
		pNorth.add(topPanel, BorderLayout.WEST);
		
		JPanel Optiondetail = new JPanel();
		Optiondetail.add(show);
		Optiondetail.add(showStrike);
		Optiondetail.add(showContract);
		Optiondetail.add(showPrice);
		
		pNorth.add(Optiondetail, BorderLayout.CENTER);
	...
	private class ChainPanel extends NewTabPanel implements IContractDetailsHandler, ActionListener  {
		...
		@Override public void contractDetails(List<ContractDetails> list) {
			...
			for (ContractDetails data : list) {
				...
				System.out.print(contract.localSymbol().toString());
				Double stockPrice= 175.0;
				if(contract.strike() == stockPrice) {
					showContract.setText(""+contract.strike());
				}
				
				
				
//				System.out.println(""+contract.();
//				System.out.println(""+contract.strike();
			}
			m_putsModel.sort();
			m_callsModel.sort();
			...
			int row = 0;
			int col =1;
			System.out.println(">>"+m_callsModel.getValueAt(row, col).toString());
		}
